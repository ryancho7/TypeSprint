...
function UpperNav({ auth, onFriendsClick }) {
  ...
  const getBtnClass = (btn) =>
    `rounded-[10px] px-6 py-2 flex items-center justify-center transition-colors duration-150 cursor-pointer group
     ${active === btn ? "bg-white text-black" : "bg-transparent hover:bg-white focus:bg-white"}`;
      ...
      <>
        <nav className="bg-black flex items-center justify-between pt-10 pl-16 pr-16 z-10">
          <div className="flex flex-row gap-[22px] items-center">
            <button className={getBtnClass("play")} onClick={handlePlayClick} type="button">
              <span className={getTextClass("play")}>PLAY</span>
            </button>
            <button className={getBtnClass("friends")} onClick={onFriendsClick} type="button">
              <span className={getTextClass("friends")}>FRIENDS</span>
            </button>
            <button className={getBtnClass("records")} onClick={handleRecordsClick} type="button">
              <span className={getTextClass("records")}>RECORDS</span>
            </button>
            <button className={getBtnClass("leaderboard")} onClick={handleLeaderboardClick} type="button">
